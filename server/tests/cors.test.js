...
describe('CORS', () => {
    ...
    it('allows the exact CLIENT_URL origin and permits credentials', async () => {
        const app = loadApp('https://dukan-app-sable.vercel.app');

        const res = await preflight(app, 'https://dukan-app-sable.vercel.app');

        expect(res.headers['access-control-allow-origin']).toBe(
            'https://dukan-app-sable.vercel.app'
        );
    ...
    it('allows each origin in a comma-separated CLIENT_URL', async () => {
        ...
        const app = loadApp(
            'https://dukan-app-sable.vercel.app, https://acc-app-staging.vercel.app'
        );
    ...
    it('allows several Vercel hostnames when Vercel hands out more than one', async () => {
        // Vercel assigns a different hostname per deployment and they change
        // unpredictably. Listing every hostname in one comma-separated
        // CLIENT_URL keeps every origin working, so a stale single value can no
        // longer break login after a redeploy.
        const app = loadApp(
            'https://dukan-app-sable.vercel.app,https://dukan-sigma.vercel.app,https://dukan-app-nine.vercel.app'
        );

        const first = await preflight(app, 'https://dukan-app-sable.vercel.app');
        expect(first.headers['access-control-allow-origin']).toBe(
            'https://dukan-app-sable.vercel.app'
        );

        const second = await preflight(app, 'https://dukan-sigma.vercel.app');
        expect(second.headers['access-control-allow-origin']).toBe(
            'https://dukan-sigma.vercel.app'
        );

        const third = await preflight(app, 'https://dukan-app-nine.vercel.app');
